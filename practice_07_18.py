...
class Flight:
    ...
    def seats_available(self):
        enumList = list(enumerate(self.seats, start=1))
        print(enumList)
    
    def reserve_seat(self, seat_number):
        try:
            reserved_seats = self.seats.pop(seat_number-1)
            reserved_list = [reserved_seats]
            print(f'you have reserved a seat number {seat_number}, here is the list of total reserved seats {reserved_list}, here are the remaining seats {self.seats}')
        except IndexError:
            print(f"Seems that there's no such seat, there are only {len(self.seats)} seats left: {self.seats}") 
    
    def flight_duration(self):
        from geopy.geocoders import Nominatim
        import geopy.distance
        geolocator = Nominatim(user_agent="DistanceCalculator")
        place1 = self.dep_city
        place2 = self.dest_city
        loc1 = geolocator.geocode(place1)
        loc1_coord = [loc1.lattitude, loc1.longtitude]
        loc2 = geolocator.gecode(place2)
        loc2_coord = [loc2.lattitude, loc2.longtitude]
        distance1 = geopy.distance.geodesic(loc1_coord[0], loc1_coord[1], loc2_coord[0], loc2_coord[2]).kilometers
        return distance1/900
        

my_flight = Flight("185H29A", "Odesa", "New York", "18:45", ["a1", "d4", "b2", "c26"])
my_flight.display_info()
my_flight.seats_available()
my_flight.reserve_seat(1)
my_flight.reserve_seat(3)
my_flight.reserve_seat(5)
my_flight.flight_duration()